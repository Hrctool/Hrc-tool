    ...
    foreach($targetUrl in $tryUrls){
        for($retry=0;$retry -lt 3;$retry++){
            try{
                ...
                while(-not $asyncTask.Wait(1000)){[System.Windows.Forms.Application]::DoEvents()}
...
if(Test-Path $AUTO_RUN_CFG){
    ...
    $hrcTitle.Font = New-Object System.Drawing.Font("Segoe UI", 56, [System.Drawing.FontStyle]::Bold)
    ...
    $glowLayer = New-Object System.Windows.Forms.Panel
    $glowLayer.Size = $bootWin.Size
    $glowLayer.Location = [System.Drawing.Point]::new(0,0)
    $glowLayer.BackColor = [System.Drawing.Color]::FromArgb(0, 180, 255)
    $glowLayer.Opacity = 0
    $glowLayer.Visible = $false
    $bootWin.Controls.Add($glowLayer)
    ...
    $bootWin.Add_Shown({
        $bootWin.Refresh()
        for($i=0;$i -le 100;$i+=3){$bootWin.Opacity = $i/100; [System.Windows.Forms.Application]::DoEvents(); Start-Sleep -Milliseconds 20}
        Start-Sleep 0.3

        $hrcTitle.Visible = $true
        for($i=0;$i -le 100;$i+=4){$hrcTitle.Opacity = $i/100; [System.Windows.Forms.Application]::DoEvents(); Start-Sleep -Milliseconds 20}
        Start-Sleep 1

        for($i=100;$i -ge 0;$i-=4){$hrcTitle.Opacity = $i/100; [System.Windows.Forms.Application]::DoEvents(); Start-Sleep -Milliseconds 20}
        $hrcTitle.Visible = $false

        $glowLayer.Visible = $true
        for($i=0;$i -le 100;$i+=3){$glowLayer.Opacity = $i/100 * 0.15; [System.Windows.Forms.Application]::DoEvents(); Start-Sleep -Milliseconds 25}
        Start-Sleep 0.5

        $winLogo.Visible = $true
        for($i=0;$i -le 100;$i+=4){$winLogo.Opacity = $i/100; [System.Windows.Forms.Application]::DoEvents(); Start-Sleep -Milliseconds 20}
        ...
        for($i=100;$i -ge 0;$i-=3){$bootWin.Opacity = $i/100; [System.Windows.Forms.Application]::DoEvents(); Start-Sleep -Milliseconds 20}
...
$bootLogo.Font = New-Object System.Drawing.Font("Segoe UI", 64, [System.Drawing.FontStyle]::Bold)
...
$bootWin.Add_Shown({
    $bootWin.Refresh()
    $totalSteps = @(
        @{p=10;name="初始化缓存目录";action={Start-Sleep -Milliseconds 200}},
        @{p=35;name="下载WinXP启动音乐";action={
            $musicPath = Join-Path $MUSIC_DIR "winxp-start.wav"
            if(-not (Test-Path $musicPath)){
                try{irm "https://cdn.jsdelivr.net/gh/ddazzyy/rc@main/winxp.wav" -OutFile $musicPath -TimeoutSec 90}catch{}
            }
        }},
        ...
        @{p=90;name="预加载系统组件";action={Start-Sleep -Milliseconds 300}},
        @{p=100;name="启动完成";action={Start-Sleep -Milliseconds 100}}
    )
...
function Show-CleanupProgress{
    ...
    $cleanTitle.Font = New-Object System.Drawing.Font($globalFont.FontFamily, 28)
    ...
    $cleanWin.Add_Shown({
        $cleanWin.Refresh()
        for($p=0;$p -lt 60;$p++){$cleanBar.Value=$p;[System.Windows.Forms.Application]::DoEvents();Start-Sleep -Milliseconds 15}
        Remove-Item $HRC_ROOT -Recurse -Force -ErrorAction SilentlyContinue
        for($p=60;$p -lt 100;$p++){$cleanBar.Value=$p;[System.Windows.Forms.Application]::DoEvents();Start-Sleep -Milliseconds 15}
        $cleanWin.Close()
    })
...
$exec.Add_Shown({
    if($musicSwitch.Checked){try{$musicPath = Join-Path $MUSIC_DIR "winxp-start.wav";(New-Object System.Media.SoundPlayer $musicPath).Play()}catch{}}
    if(-not $isWin7){
